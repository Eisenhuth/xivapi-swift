...
struct Item_Tests {
    ...
    @Test("Item: Tincture") func ItemTincture() async throws {
        ...
        #expect(item.icon.iconUrl(format: .jpg)?.description ==  "\(xivapi.baseUrl)/asset?path=ui/icon/020000/020710.tex&format=jpg")
        #expect(item.icon.iconUrlHD(format: .jpg)?.description == "\(xivapi.baseUrl)/asset?path=ui/icon/020000/020710_hr1.tex&format=jpg")
